...
But let's disregard this advice and start off our brute force solution by considering our pendulum. The force diagram below illustrates the problem setup. Parameter ${t`k`} parameterizes the kinetic energy of the pendulum, from rest (${t`k = 0`}) to a pendulum with exactly enough energy to start at the bottom and come to rest at the top (${t`k = 1`}).`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:void 0});s({root:document.getElementById("cell-671"),expanded:[],variables:[]},{id:671,body:function(e,t,n,r,o,u,d,c,b){const f=e`The physical setup for a simple gravity pendulum with kinetic energy parameter ${t`k = ${n}`}.`,x=1e3,y=1,m=9.98;function k(h){return 2*Math.asin(n*r(h*Math.sqrt(m/y),n*n))}const $=Math.min(o,640),a=80,l=m/9.81,v=300,p=u.create("svg").attr("width",$).attr("height",v+a).attr("style","border:1px solid #eee;cursor:move"),w=p.append("g").attr("transform",`translate(${$/2},${v/2})`);w.append("line").attr("y2",50).attr("stroke","#555").attr("stroke-dasharray","3,3");const E="#25e",K=w.append("path").attr("fill","#555");w.append("circle").attr("r",v/2*y*.9).style("stroke","#aaa").style("fill","none").style("stroke-dasharray","4,4");const P=w.append("path").attr("fill","#555"),C=w.append("text").text("θ").attr("dx",2).attr("dy",15).style("font-family","KaTeX_Math"),M=[k].map(h=>{const i={theta:h,line:w.append("line").attr("y2",v/2*y*.9).style("stroke",h===k?"#444":"#ccc").style("stroke-width",3).style("stroke-linecap","round"),mass:w.append("g").attr("transform",`translate(0,${v/2*y*.9})`)};return i.radArrowhead=i.mass.append("path").attr("fill",E),i.radialForce=i.mass.append("line").attr("x2",0).attr("y2",a*l).attr("stroke",E).attr("stroke-width",2),i.force=i.mass.append("line").attr("x2",0).attr("y2",a*l-5).attr("stroke","#e25").attr("stroke-width",2),i.gArrowhead=i.mass.append("path").attr("fill","#e25").attr("d",d(0,0,0,a*l)),i.tanArrowhead=i.mass.append("path").attr("fill",E),i.torqueForce=i.mass.append("line").attr("x2",0).attr("y2",a*l).attr("stroke",E).attr("stroke-width",2),i.circle=i.mass.append("circle").attr("r",10).style("stroke","white").style("stroke-width",1).style("fill",h===k?"#e25":"#ccc"),i.text=i.mass.append("text").text("-mg").style("font-family","KaTeX_Math").attr("dy",10).style("text-anchor","middle").attr("y",a*l),i.lText=w.append("text").text("l").style("font-family","KaTeX_Math").style("visibility","hidden"),i.sinText=i.mass.append("text").style("font-family","KaTeX_Math").attr("dx",3).attr("dy",-3),i.sinText.style("visibility","hidden"),i.sinText.append("tspan").text("-mg "),i.sinText.append("tspan").style("font-family","KaTeX_Main").text("sin"),i.sinText.append("tspan").text(" θ"),i});p.append("g");const X=k;function z(h,i=null){const L=h/x;M.forEach(_=>{const B=i===null?X(L):i,[T,g]=[Math.cos(B),Math.sin(B)],R=v/2*y*.9*g,j=v/2*(y*.9*T);_.line.attr("x2",R).attr("y2",j),_.mass.attr("transform",`translate(${R},${j})`),_.lText.attr("x",R/2).attr("y",j/2).attr("dx",10*-T).attr("dy",15*g).attr("text-anchor","middle").attr("alignment-baseline","middle");const S=Math.sign(-g*T);_.radialForce.attr("x1",S*g*g*l*5).attr("y1",S*T*g*l*5+a*l).attr("x2",-a*g*T*l).attr("y2",a*g*g*l),_.torqueForce.attr("x2",-75*T*g*l).attr("y2",(a-5)*g*g*l),_.tanArrowhead.attr("d",d(0,0,-a*T*g*l,a*g*g*l)),_.radArrowhead.attr("d",d(-a*T*g*l,a*g*g*l,0,a*l));const V=g;_.sinText.attr("x",-a*V*T*l).attr("y",a*V*g*l).attr("dx",S>0?3:-3).attr("text-anchor",S>0?"start":"end");const Y=Math.cos(Math.max(B,0)),D=Math.sin(Math.max(B,0));P.attr("d",d(D*15.25-Y,Y*15.25+D,D*15.5,Y*15.5,{length:8,width:7,inset:1.5}));const H=Math.max(B,0)*.5;C.attr("x",Math.sin(H)*15.5).attr("y",Math.cos(H)*15.5)}),K.attr("d",u.arc().innerRadius(15).outerRadius(16).startAngle(Math.PI).endAngle(Math.PI-(i===null?X(L):i))),i===null&&(q=requestAnimationFrame(z))}function A(h){const i=p.node().getBoundingClientRect(),L=h.clientX-i.left-$/2,_=h.clientY-i.top-v/2;z(null,Math.atan2(L,_))}function J(){p.node().addEventListener("mousemove",A),M[0].sinText.style("visibility","visible"),M[0].lText.style("visibility","visible"),cancelAnimationFrame(q),q=null}function O(){M[0].sinText.style("visibility","hidden"),M[0].lText.style("visibility","hidden"),p.node().removeEventListener("mousemove",A),q===null&&(q=requestAnimationFrame(z))}function U(h){J(),h.preventDefault(),h.stopPropagation(),A({clientX:h.touches[0].clientX,clientY:h.touches[0].clientY})}function W(h){h.preventDefault(),h.stopPropagation(),A({clientX:h.touches[0].clientX,clientY:h.touches[0].clientY})}function N(h){h.touches.length||O()}p.node().addEventListener("mouseover",J),p.node().addEventListener("mouseout",O),p.node().addEventListener("touchstart",U),p.node().addEventListener("touchmove",W),p.node().addEventListener("touchend",N);let F;setTimeout(()=>{F=new c(function([h]){h.isIntersecting?q||(q=requestAnimationFrame(z)):(cancelAnimationFrame(q),q=null)}),F.observe(p.node())},1),b.then(()=>{cancelAnimationFrame(q),F&&F.disconnect()});let q=requestAnimationFrame(z);return e`<figure>
    ${p.node()}
...
Recall that ${t`\mathrm{sech}(u) \equiv 1/\mathrm{cosh}(u).`} So in this sense they sort of interpolate between the [trigonometric](https://en.wikipedia.org/wiki/Trigonometric_functions) and [hyperbolic](https://en.wikipedia.org/wiki/Hyperbolic_functions) functions. Very cool. See [Jacobi Elliptic Functions in the Complex Plane](https://observablehq.com/@rreusser/jacobi-elliptic-functions-in-the-complex-plane) for an even lovelier view of how this generalization works.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:void 0});s({root:document.getElementById("cell-769"),expanded:[],variables:[]},{id:769,body:(e,t,n,r,o,u,d,c,b,f,x)=>{let y;e<1?y=t(e):e===1?y=10:y=t(1/e)/Math.sqrt(e);const m=501,k=[...Array(m).keys()].map(a=>(a/(m-1)*5-.5)*y),$=k.map(a=>{const l=a/y;let v,p,w;v=n(a,e),p=r(a,e),w=o(a,e);const E=u(a,e),K=p*p+(1-e)*v*v-w*w;return{am:E,uK:l,u:a,sn:v,cn:p,dn:w,am:E,checksum:K}});return d(c.html`<figure style="max-width:100%">${b.plot({width:Math.min(640,f),height:300,grid:!0,x:{label:"u / K →",ticks:4},y:{},marks:[b.ruleY([0]),b.line($,{x:"uK",y:"sn",stroke:"#c39"}),b.line($,{x:"uK",y:"cn",stroke:"#9c3"}),b.line($,{x:"uK",y:"dn",stroke:"#39c"})].filter(a=>a)})}<figcaption>
