import{d as s,_ as G}from"./index-ByB2dbry.js";s({root:document.getElementById("cell-23"),expanded:[],variables:[]},{id:23,body:(a,t)=>a`## The 1D Wave Equation
...
\frac{\partial v_x}{\partial t} &= c \frac{\partial u}{\partial x} - \sigma_x v_x \\[10pt]
\frac{\partial v_y}{\partial t} &= c \frac{\partial u}{\partial y} - \sigma_y v_y
\end{aligned}`}`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:void 0});s({root:document.getElementById("cell-49"),expanded:[],variables:[]},{id:49,body:(a,t)=>a`These equations require tracking three fields: the scalar field ${t`u`}, the auxiliary field ${t`\psi`}, and the vector field ${t`\mathbf{v} = (v_x, v_y)`}. In practice, this naturally lends itself to a two-buffer implementation: one storing ${t`(u, \psi)`} and the other storing ${t`(v_x, v_y)`}.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:void 0});s({root:document.getElementById("cell-50"),expanded:[],variables:[]},{id:50,body:(a,t)=>a`## Implementation

The following WebGL implementation simulates these equations on the GPU using fragment shaders. Use the controls below to pause/resume the simulation, adjust contrast, and visualize the PML absorption regions (red for ${t`\sigma_x`}, green for ${t`\sigma_y`}).`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:void 0});s({root:document.getElementById("cell-17"),expanded:[],variables:[]},{id:17,body:a=>{const t=a`<canvas width="800" height="800" style="width: 100%; max-width: 800px; height: auto; display: block; image-rendering: pixelated;"></canvas>`,i=t.getContext("webgl2");if(!i)throw new Error("WebGL 2 is not supported");const A=i.getExtension("EXT_color_buffer_float");if(!A)throw new Error("EXT_color_buffer_float extension is not supported");const M=12,W=2,d=1,D=4,e=10,h=1,m=.5,y=1,L=`#version 300 es
    in vec2 a_position;
    void main() {
