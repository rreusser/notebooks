import"./index-DLwE0_Ng.js";import{d}from"./define-D1bHXsD4.js";d({root:document.getElementById("cell-1771"),expanded:[],variables:[]},{id:1771,body:(t,e)=>t`The [Baily-Borwein-Plouffe](https://en.wikipedia.org/wiki/Bailey%E2%80%93Borwein%E2%80%93Plouffe_formula) (BBP) formula is a remarkable formula for computing the hexadecimal digits of ${e`\pi`}, starting at the ${e`n^{th}`} digit, *without first computing preceeding digits*!`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1820"),expanded:[],variables:[]},{id:1820,body:t=>t.block`\pi =\sum _{k=0}^{\infty }\left[{\frac {1}{16^{k}}}\left({\frac {4}{8k+1}}-{\frac {2}{8k+4}}-{\frac {1}{8k+5}}-{\frac {1}{8k+6}}\right)\right].`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1770"),expanded:[],variables:[]},{id:1770,body:(t,e)=>t`That such a formula could exist is a bit shocking—though the initial shock is tempered by the realization that it does, of course, take longer as you start at later digits (${e`O(n \log n)`} as a function of the starting digit, to be precise). Additionally, the hexadecimal digits cannot be converted to decimal without first computing all of the leading digits. What it does enable though, is to compute digits deep into ${e`\pi`} with hardly any memory consumption and nothing more than a basic double precision data type.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1193"),expanded:[],variables:[]},{id:1193,body:function*(e,i,a){if(!this){yield e`Click to compute π starting at the millionth hexadecimal digit.`;return}yield e`Computing…`;const n=performance.now(),u=10,o=a(1e6,u),c=performance.now();yield e`<div>
    Hexadecimal digits of π starting at the millionth (computed in ${((c-n)/1e3).toFixed(2)} seconds):<br><code style="color:rgb(32, 165, 186)">${o.toString(16).padStart(u,"0")}</code>
  </div>`},inputs:["html","recompute","computePi"],outputs:void 0,output:"viewof$dummy",assets:void 0,autodisplay:!0,autoview:!0,automutable:!1});d({root:document.getElementById("cell-1242"),expanded:[],variables:[]},{id:1242,body:(t,e)=>({recompute:t(e.button("Compute"))}),inputs:["view","Inputs"],outputs:["recompute"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1774"),expanded:[],variables:[]},{id:1774,body:(t,e)=>t`The table below shows hexadecimal digits of ${e`\pi`}. Each line represents an independent run of the algorithm starting at a slightly different digit. Green digits are correctly computed digits, while red represents incorrect digits. Due to our termination criteria, we usually get a couple fewer digits than requested.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-805"),expanded:[],variables:[]},{id:805,body:(t,e)=>({method:t(e.radio(["Floating point","BigInt"],{label:"Data type",value:"Floating point"}))}),inputs:["view","Inputs"],outputs:["method"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-333"),expanded:[],variables:[]},{id:333,body:(t,e,i)=>({startingDigit:t(e.range([0,i.referenceDigits.length-128],{value:0,label:"Starting digit",step:1}))}),inputs:["view","Inputs","reference"],outputs:["startingDigit"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-437"),expanded:[],variables:[]},{id:437,body:(t,e,i)=>({digits:t(e.range([2,i==="Floating point"?14:128],{label:"Digits to compute",value:i==="Floating point"?14:128,step:1}))}),inputs:["view","Inputs","method"],outputs:["digits"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1821"),expanded:[],variables:[]},{id:1821,body:t=>({figureWidth:Math.min(950,window.innerWidth+1e-8*t)}),inputs:["width"],outputs:["figureWidth"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-478"),expanded:[],variables:[]},{id:478,body:function*(e,i,a,n,u,o,c,b,r){const s=Math.floor((e-90)/850*128),f=i.referenceDigits.slice(a,a+s);function I(m,l,h){const y=document.createElement("span"),w=document.createElement("span");w.textContent=a===0&&h>0?f[0]+"."+f.slice(1,h):f.slice(0,h),y.appendChild(w),y.style.color="#ccc";for(let g=0;g<f.length-h;g++){const p=document.createElement("span");p.textContent=l[g],y.appendChild(p);const B=m[g]===l[g]?"rgb(32, 165, 186)":"#c12";if(p.style.color=B,p.style.fontWeight=900,a===0&&h===0&&g===0){const k=document.createElement("span");k.textContent=".",k.style.color=B,y.appendChild(k)}}const v=document.createElement("span");return v.textContent=f.slice(h+n,f.length),y.appendChild(v),y.style.color="#bbb",y}const x=[],E=a===0?f[0]+"."+f.slice(1):f;for(let m=a;m<a+f.length;m+=4){const l=document.createElement("code");l.style.fontSize="inherit",l.style.color="#ccc",l.textContent=E,x.push(l)}const $=x.map(m=>u`<code style="font-size:inherit;">${m}</code><br>`),_=Math.min(0,-(e-40-o)/2),P=u`
    <pre style="padding:0;font-size:${11}px;background:white;width:${e}px;margin-left:${_}px">
${$}</pre>
  `,S=c==="Floating point"?b:r;for(let m=0,l=a;l<a+f.length;m++,l+=4){const h=performance.now(),y=S(l,n),w=performance.now(),v=y.toString(16).padStart(n,"0"),g=I(i.referenceDigits.slice(l,l+n),v,l-a),p=document.createElement("code");p.style.fontSize="inherit",p.style.color="rgb(32, 165, 186)",p.style.fontWeight="700",p.textContent=` (${(w-h).toFixed(1)}ms)`,g.appendChild(p),x[m].replaceWith(g),yield P}},inputs:["figureWidth","reference","startingDigit","digits","html","width","method","computePi","computePiBigInt"],outputs:void 0,output:"table",assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1822"),expanded:[],variables:[]},{id:1822,body:function(){return{referenceDigits:`
`.split(`
`).join("")}},inputs:[],outputs:void 0,output:"reference",assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1779"),expanded:[],variables:[]},{id:1779,body:(t,e)=>t`So let's walk through using the identity to compute ${e`\pi`} starting at an arbitrary location. I'll largely repeat the eponymous David Bailey's explanation from *[The BBP Algorithm for Pi](https://www.davidhbailey.com/dhbpapers/bbp-alg.pdf)*, filling in some details I learned along the way. The implemenation is also heavily influenced by the version from [literateprograms.org](https://literateprograms.org/pi_with_the_bbp_formula__python_.html).`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1781"),expanded:[],variables:[]},{id:1781,body:t=>t.block`\pi =\sum_{k=0}^{\infty }\left[{\frac {1}{16^{k}}}\left({\frac {4}{8k+1}}-{\frac {2}{8k+4}}-{\frac {1}{8k+5}}-{\frac {1}{8k+6}}\right)\right].`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1782"),expanded:[],variables:[]},{id:1782,body:(t,e)=>t`We first multiply through by ${e`16^d`}, then take the fractional part (modulo 1, if you prefer) which we denote by ${e`\{\cdot\}.`}`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1783"),expanded:[],variables:[]},{id:1783,body:t=>t.block`\{16^d\pi\} = \left\{\sum _{k=0}^{\infty }\left(
{\frac {16^{d-k} 4}{8k+1}}-
{\frac {16^{d-k} 2}{8k+4}}-
{\frac {16^{d-k}}{8k+5}}-
{\frac {16^{d-k}}{8k+6}}
\right)\right\}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1784"),expanded:[],variables:[]},{id:1784,body:(t,e)=>t`Consider ${e`d = 4`} (starting at the fourth digit):`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1785"),expanded:[],variables:[]},{id:1785,body:t=>t.block`\begin{aligned}
\pi &\approx \texttt{3.243f6a8885a368d31\ldots}_{16} \\
16^4 \pi &\approx \texttt{3243f.6a8885a368d31\ldots}_{16} \\
\left\{16^d \pi\right\} = \left\{16^4 \pi\right\} &\approx \phantom{0000\,}\texttt{0.6a8885a368d31\ldots}_{16}
\end{aligned}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1786"),expanded:[],variables:[]},{id:1786,body:(t,e)=>t`In hexadecimal, ${e`16^d`} shifts the value ${e`d`} places to the left (analogous to multiplication by ten in base ten), while the fractional part operation chops off any leading digits to the left of the [radix point](https://en.wikipedia.org/wiki/Radix_point). Thus, it pretty directly gives us the hexadecimal representation of ${e`\pi`} starting at the ${e`d^{th}`} digit.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1787"),expanded:[],variables:[]},{id:1787,body:(t,e)=>t`If we then multiply by ${e`16^n`} and take the integer part, we obtain ${e`n`} digits of ${e`\pi`}, starting at digit ${e`d`}. Consider ${e`n = 6`}:`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1788"),expanded:[],variables:[]},{id:1788,body:t=>t.block`\begin{aligned}
\left\{16^4 \pi\right\} &\approx \texttt{0.6a8885a368d31\ldots}_{16} \\
16^6 \left\{16^4 \pi\right\} &\approx \phantom{0\;\;} \texttt{6a8885.a368d31\ldots}_{16} \\
\lfloor 16^n \left\{16^d \pi\right\} \rfloor = \lfloor 16^6 \left\{16^4 \pi\right\} \rfloor &\approx \phantom{0\;\;} \texttt{6a8885}_{16}
\end{aligned}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1790"),expanded:[],variables:[]},{id:1790,body:(t,e)=>t`Our key observation is that, in order to keep numbers small, the fractional part operation can be distributed into each term of the summation. This works since, for example, ${e`\left\{3.4 + 5.1\right\} = \left\{\left\{3.4\right\} + \left\{5.1\right\}\right\} = \left\{0.4 + 0.1\right\} = 0.5`}, and yields`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1791"),expanded:[],variables:[]},{id:1791,body:t=>t.block`\{16^d\pi\} = \left\{\sum _{k=0}^{\infty }\left(
\left\{\frac {16^{d-k} 4}{8k+1}\right\}-
\left\{\frac {16^{d-k} 2}{8k+4}\right\}-
\left\{\frac {16^{d-k}}{8k+5}\right\}-
\left\{\frac {16^{d-k}}{8k+6}\right\}
\right)\right\}.`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1792"),expanded:[],variables:[]},{id:1792,body:(t,e)=>t`To simplify—and realizing that we can move factors like ${e`2`} and ${e`4`} around as well—we break this up and write`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1793"),expanded:[],variables:[]},{id:1793,body:t=>t.block`\left\{16^d \pi\right\} = \left\{
4\left\{ 16^d S_1 \right\} -
2\left\{ 16^d S_4 \right\} -
\left\{ 16^d S_5 \right\} -
\left\{ 16^d S_6 \right\}
\right\},`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1795"),expanded:[],variables:[]},{id:1795,body:t=>t.block`S_j = \sum_{k=0}^\infty \frac{1}{16^k(8k+j)}.`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1796"),expanded:[],variables:[]},{id:1796,body:(t,e)=>t`To separate the positive (growing) and negative (quickly shrinking) powers of ${e`16`}, we can split each summation into two parts, from ${e`0`} to ${e`d`} and from ${e`d + 1`} to ${e`\infty`}.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1797"),expanded:[],variables:[]},{id:1797,body:t=>t.block`\begin{aligned}
\left\{16^d S_j \right\} &=
\left\{\left\{ \sum_{k=0}^d \frac{16^{d-k}}{8k + j}\right\} +
\sum_{k=d+1}^\infty \frac{16^{d-k}}{8k+j}\right\}
\end{aligned}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1799"),expanded:[],variables:[]},{id:1799,body:t=>t.block`\begin{aligned}
\left\{16^d S_j\right\} &=
\left\{\left\{ \sum_{k=0}^d \frac{16^{d-k} \operatorname{mod} 8k+j}{8k + j}\right\} +
\sum_{k=d+1}^\infty \frac{16^{d-k}}{8k+j}\right\}
\end{aligned}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1460"),expanded:[],variables:[]},{id:1460,body:(t,e)=>t`The second term gets small quickly as ${e`k`} increases. As for the first term, expressions of the form ${e`b^e \operatorname{mod} m`} are called [*modular exponentiation*](https://en.wikipedia.org/wiki/Modular_exponentiation).`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1803"),expanded:[],variables:[]},{id:1803,body:(t,e)=>t`${e.block`
3^{17} = 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3 \cdot 3.
`}`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1804"),expanded:[],variables:[]},{id:1804,body:(t,e)=>t`We can multiply all the ${e`3`}'s, but it's rather wasteful. Instead, observe that`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1805"),expanded:[],variables:[]},{id:1805,body:t=>t.block`\begin{aligned}
3^{17} &= 3^{16} \cdot 3 \\
&= (3^8)^2 \cdot 3 \\
&= ((3^4)^2)^2 \cdot 3 \\
&= (((3^2)^2)^2)^2 \cdot 3 \\
\end{aligned}`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1480"),expanded:[],variables:[]},{id:1480,body:(t,e)=>t`By repeatedly squaring, we can immediately reduce the number of multiplications from sixteen to five. As an algorithm, we repeatedly square the base as we step through the bits of the exponent. Here, the binary representation of ${e`17`} is ${e`10001_2`}. If the ${e`n^{th}`} bit is a 1, we multiply by the ${e`n`}-times squared base.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1498"),expanded:[],variables:[]},{id:1498,body:()=>{function t(e,i){let a=1;for(;i>0;)i&1&&(a*=e),e*=e,i>>=1;return a}return{binaryPow:t}},inputs:[],outputs:["binaryPow"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1685"),expanded:[],variables:[]},{id:1685,body:t=>t(3,17),inputs:["binaryPow"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1806"),expanded:[],variables:[]},{id:1806,body:(t,e)=>t`*[Modular exponentiation](https://en.wikipedia.org/wiki/Modular_exponentiation)* computes the exponentiation ${e`\operatorname{mod} m`},`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1807"),expanded:[],variables:[]},{id:1807,body:t=>t.block`b^e \operatorname{mod} m.`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1808"),expanded:[],variables:[]},{id:1808,body:(t,e)=>t`While ${e`b^e`} may grow very large, ${e`b^e \operatorname{mod} m`} will always be less than ${e`m`}. An efficient algorithm, based on binary exponentiation, makes use of [the fact that](https://en.wikipedia.org/wiki/Modular_exponentiation#Memory-efficient_method)`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1809"),expanded:[],variables:[]},{id:1809,body:t=>t.block`
(a \cdot b) \operatorname{mod} m = [(a \operatorname{mod} m) \cdot (b \operatorname{mod} m)] \operatorname{mod} m.`,inputs:["tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1017"),expanded:[],variables:[]},{id:1017,body:(t,e)=>t`Put simply, we just load the \`binaryPow\` function up with ${e`\operatorname{mod} m`} and keep our result between ${e`0`} and ${e`m`}.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-832"),expanded:[],variables:[]},{id:832,body:()=>{function t(e,i,a){e=e%a;let n=1;for(;i>0;)i&1&&(n=n*e%a),e=e*e%a,i>>=1;return n}return{modPow:t}},inputs:[],outputs:["modPow"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1700"),expanded:[],variables:[]},{id:1700,body:function(t,e){return t`Consider ${e`3^{17} \operatorname{mod} 50`}:`},inputs:["md","tex"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1695"),expanded:[],variables:[]},{id:1695,body:t=>t(3,17,50),inputs:["modPow"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1706"),expanded:[],variables:[]},{id:1706,body:function(t){return t`Compare with`},inputs:["md"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1697"),expanded:[],variables:[]},{id:1697,body:()=>3**17%50,inputs:[],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1487"),expanded:[],variables:[]},{id:1487,body:function(t,e){return t`
As one final prelude, we implement an alternative to the JavaScript \`%\` operator since its behavior with negative numbers is somewhat undesirable. \`-1.6 % 1\` is equal to ${e`-0.6`}, but we prefer the range ${e`[0, 1)`} instead.
`},inputs:["md","tex"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-829"),expanded:[],variables:[]},{id:829,body:()=>{function t(e,i){return e%=i,e<0?e+i:e}return{mod:t}},inputs:[],outputs:["mod"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1501"),expanded:[],variables:[]},{id:1501,body:(t,e)=>t`The function below directly implements the above summation ${e`S_j`} and the function \`computePi(d, n)\` which starts at digit ${e`d`} and computes ${e`n`} digits of ${e`\pi`}. It returns the result as an integer JavaScript \`Number\` so that we'll need to run \`.toString(16)\` to convert the output to hexadecimal.`,inputs:["md","tex"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-792"),expanded:[],variables:[]},{id:792,body:(t,e)=>{function i(n,u){let o=0;for(let b=0;b<=u;b++){let r=8*b+n;o=t(o+e(16,u-b,r)/r,1)}let c=0;for(let b=u+1;;b++){let r=c+16**(u-b)/(8*b+n);if(c===r)break;c=r}return o+c}function a(n,u){return n-=1,Math.floor(16**u*t(4*i(1,n)-2*i(4,n)-i(5,n)-i(6,n),1))}return{S:i,computePi:a}},inputs:["mod","modPow"],outputs:["S","computePi"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1768"),expanded:[],variables:[]},{id:1768,body:t=>t(0,10).toString(16),inputs:["computePi"],outputs:[],output:void 0,assets:void 0,autodisplay:!0,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1512"),expanded:[],variables:[]},{id:1512,body:function(t){return t`## BigInt implementation`},inputs:["md"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1769"),expanded:[],variables:[]},{id:1769,body:()=>{function t(a,n,u){if(!n)return 1n;a=a%u;let o=1n;for(;;){if(n&1n&&(o=o*a%u),n>>=1n,!n)return o;a=a*a%u}}function e(a,n,u,o){const c=u<<2n;let b=0n;for(let s=0n;s<=n;s++){let f=s*8n+a;b=b+(t(16n,n-s,f)<<c)/f&o}let r=0n;for(let s=n+1n;;s++){let f=r+16n**(u+n-s)/(s*8n+a);if(r===f)break;r=f}return b+r}function i(a,n){const u=BigInt(n),o=BigInt(a)-1n,c=16n**u-1n;return 4n*e(1n,o,u,c)-2n*e(4n,o,u,c)-e(5n,o,u,c)-e(6n,o,u,c)&c}return{modPowBigInt:t,SBigInt:e,computePiBigInt:i}},inputs:[],outputs:["modPowBigInt","SBigInt","computePiBigInt"],output:void 0,assets:void 0,autodisplay:!1,autoview:void 0,automutable:void 0});d({root:document.getElementById("cell-1739"),expanded:[],variables:[]},{id:1739,body:function(t){return t`The article from literateprograms.org, [Pi with the BBP formula](https://literateprograms.org/pi_with_the_bbp_formula__python_.html), notes that there's an additional, more efficient variation that generates digits sequentially, but since it requires a rational data type, I've not implemented it.`},inputs:["md"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});d({root:document.getElementById("cell-1521"),expanded:[],variables:[]},{id:1521,body:function(t){return t`## References

<div class="references">${t`
0. Bailey, D. H., *[The BBP Algorithm for Pi](https://www.davidhbailey.com/dhbpapers/bbp-alg.pdf).*
0. Bailey, D. H., [PSLQ: An Algorithm to Discover Integer Relations](https://www.davidhbailey.com/dhbpapers/pslq-comp-alg.pdf).
0. *[Pi with the BBP formula](https://literateprograms.org/pi_with_the_bbp_formula__python_.html)*, literateprograms.org.
0. *[Modular exponentiation](https://en.wikipedia.org/wiki/Modular_exponentiation)*, wikipedia.org.
`}`},inputs:["md"],outputs:void 0,output:void 0,assets:void 0,autodisplay:!0,autoview:!1,automutable:!1});
