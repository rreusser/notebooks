 ...
 */
export function generateUnscrambleShader(N: number, R: number, C: number): string {
  return `
 ...
 */
export function generateScrambleShader(N: number, R: number, C: number): string {
  return `
 ...
 */
export function generateTwiddleShader(N: number, R: number, C: number): string {
  return `
 ...
 */
export function generateSubRowFFTShader(C: number, R: number): string {
  if (C < 2 || (C & (C - 1)) !== 0) {
 ...
 */
export function generateSubRowTwiddleShader(N: number, R: number, C: number): string {
  return `
 ...
 */
export function generateSubRowColumnFFTShader(R: number, C: number): string {
  if (R < 2 || (R & (R - 1)) !== 0) {
  ...
  if (R <= 8) {
    return generateSmallSubRowColumnFFT(R, C);
  }
...
function generateSmallSubRowColumnFFT(R: number, C: number): string {
  const N = R * C;
 ...
 */
export function generateColumnFFTShader(R: number): string {
  if (R < 2 || (R & (R - 1)) !== 0) {
    throw new Error(`R must be a power of 2, got ${R}`);
  }
 ...
 */
export function generateSmallColumnFFTShader(R: number): string {
  if (R === 2) {
    return generate2PointColumnFFT();
  } else if (R === 4) {
    return generate4PointColumnFFT();
  } else if (R === 8) {
    return generate8PointColumnFFT();
  }
  throw new Error(`Unsupported small R: ${R}. Use 2, 4, or 8.`);
}

function generate2PointColumnFFT(): string {
  return `
...
fn column_fft(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let x0 = input[col];          // row 0
  let x1 = input[C + col];      // row 1
...
function generate4PointColumnFFT(): string {
  return `
...
fn column_fft(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let x0 = input[col];
  let x1 = input[C + col];
  let x2 = input[2u * C + col];
  let x3 = input[3u * C + col];
...
function generate8PointColumnFFT(): string {
  return `
...
fn column_fft(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let x0 = input[col];
  let x1 = input[4u * C + col];
  let x2 = input[2u * C + col];
  let x3 = input[6u * C + col];
  let x4 = input[C + col];
  let x5 = input[5u * C + col];
  let x6 = input[3u * C + col];
  let x7 = input[7u * C + col];
