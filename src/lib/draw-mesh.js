// Creates a mesh drawing command
// Usage: createDrawMesh(regl) => drawMesh command
export function createDrawMesh(regl) {
  return regl({
    vert: `
    ...
    uniforms: {
      ...
      uBorderWidth: (ctx, props) => (props.borderWidth || 1.5) * ctx.pixelRatio,
      uBorderColor: (ctx, props) => props.borderColor || [0.1, 0.2, 0.3, 1.0],
      uDrawOpenEdges: (ctx, props) => props.drawOpenEdges ? 1.0 : 0.0,
    },
    attributes: {
