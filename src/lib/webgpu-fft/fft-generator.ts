...
/**
 * Generate vec4 FFT shader that processes two complex numbers per element.
 * Used for inverse FFT of convolution results where we have 4 real outputs packed as (re1, im1, re2, im2).
 */
export function generateVec4FFTShader(N: number, precision: FFTPrecision = 'f32'): string {
  if (N == null || !Number.isInteger(N) || N < 2 || (N & (N - 1)) !== 0) {
    throw new Error(`N must be a power of 2 integer >= 2, got ${N}`);
  }

  const log2N = Math.log2(N);
  const storageType = precision === 'f16' ? 'vec4<f16>' : 'vec4<f32>';
  const enableF16 = precision === 'f16' ? 'enable f16;\n' : '';
  const loadConvert = precision === 'f16' ? 'vec4<f32>' : '';
  const storeConvert = precision === 'f16' ? 'vec4<f16>' : '';

  return `
// Generated Vec4 FFT Shader for N=${N}
...
var<workgroup> buffer_a: array<vec4<f32>, ${N}>;
var<workgroup> buffer_b: array<vec4<f32>, ${N}>;
...
/**
 * Generate vec4 transpose shader
 */
export function generateVec4TransposeShader(precision: FFTPrecision = 'f32'): string {
  const storageType = precision === 'f16' ? 'vec4<f16>' : 'vec4<f32>';
  const enableF16 = precision === 'f16' ? 'enable f16;\n' : '';
  const loadConvert = precision === 'f16' ? 'vec4<f32>' : '';
  const storeConvert = precision === 'f16' ? 'vec4<f16>' : '';
