 ...
 * @param precision Storage precision: 'f16' for half-precision (requires shader-f16), 'f32' for single precision
 */
export function createLargeFFTPipelines(
  ...
  const transposeCode = generateTransposeShader(precision);
  ...
  if (!isLarge) {
    ...
    const simpleFFTCode = generateFFTShader(N, precision);
  ...
  console.log(`Large FFT: N=${N} = ${R}×${C}, maxWorkgroup=${maxWorkgroupSize}, precision=${precision}`);

  // Generate shaders for hierarchical FFT
  const subRowFFTCode = generateSubRowFFTShader(C, R, precision);
  const twiddleCode = generateSubRowTwiddleShader(N, R, C, precision);
  const columnFFTCode = generateSubRowColumnFFTShader(R, C, precision);
  const unscrambleCode = generateUnscrambleShader(N, R, C, precision);
  const scrambleCode = generateScrambleShader(N, R, C, precision);

  // Also create simple FFT for C-point transforms (used in 2D after transpose)
  const simpleFFTCode = generateFFTShader(C, precision);
...
function executeHierarchical2DFFT(
  ...
  // Step H1: Column FFT (R-point) on input → temp[0]
  {
    ...
    if (R <= 8) {
      ...
      // 2D dispatch: x = columns (C), y = original rows (N)
  ...
  // Step H3: Sub-row FFT (C-point) on temp[1] → temp[0]
  {
    ...
    pass.setBindGroup(0, bindGroup);
    // 2D dispatch: x = sub-row within original row (R), y = original rows (N)
