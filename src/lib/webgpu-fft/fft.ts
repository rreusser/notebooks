 ...
 */
export function createFFTPipelines(device: GPUDevice, N: number, precision: FFTPrecision = 'f32'): FFTPipelines {
  if (N < 2 || (N & (N - 1)) !== 0) {
  ...
  if (isLarge) {
    ...
    const largePipelines = createLargeFFTPipelines(device, N, maxWorkgroupSize, precision);
  ...
  const fftCode = generateFFTShader(N, precision);
