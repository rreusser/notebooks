...
export function createZoomableAxes({
  ...
  scaleExtent = [0.1, 100],
  flipY = false,  // If true, flip Y axis so Y=0 is at top of screen (screen coordinates)
  onChange = () => {}
}) {
  ...
  function updateMatrices() {
    // When flipY is true, swap bottom/top to flip Y axis in clip space
    // This makes Y=0 appear at the top of the screen (standard screen coordinates)
    const bottom = flipY ? yDomain[1] : yDomain[0];
    const top = flipY ? yDomain[0] : yDomain[1];
    ortho(view, xDomain[0], xDomain[1], bottom, top, -1, 1);
