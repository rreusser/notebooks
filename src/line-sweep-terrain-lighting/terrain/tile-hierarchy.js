 ...
 * @returns {{x: number, y: number, z: number}} Parent tile coordinates
 */
 ...
 * @returns {'nw' | 'ne' | 'sw' | 'se'} Quadrant identifier
 */
 ...
 *
 * @param {{x: number, y: number, z: number}} target - Target tile coordinates
 * @returns {Array<{x: number, y: number, z: number, role: string}>}
 */
export function getTileSet(target) {
  const { x, y, z } = target;
